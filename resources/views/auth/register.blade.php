...
<div class="h-screen bg-white flex flex-col space-y-10 justify-center items-center">
    <div class="bg-white w-96 shadow-xl rounded p-5">
        ...
        <form action="{{route('register_process')}}" class="space-y-5 mt-5" method="POST">
            @csrf
            <input name="name" type="text" class="w-full h-12 border border-gray-800 rounded px-3 @error('email') text-red-500 @enderror" placeholder="Name" />
            @error('name')
            ...
            <p class="text-red-500">{{$message}}</p>
            @enderror
            <input name="password" type="password" class="w-full h-12 border border-gray-800 rounded px-3 @error('email') text-red-500 @enderror" placeholder="Пароль" />
            @error('password')
            <p class="text-red-500">{{$message}}</p>
            @enderror
            <input name="password_confirmation" type="password" class="w-full h-12 border border-gray-800 rounded px-3 @error('email') text-red-500 @enderror" placeholder="Подтверждение пароля" />
